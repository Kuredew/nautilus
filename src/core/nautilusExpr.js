...
export function applyLayer(layer, config) {
  ...
  const finalExpr = exprList.map((expr) =>
    getExpr(nautilus.expression.layer.template, {
...
export function applyTextLayer(textLayer, effectName) {
  try {
    ...
    const finalPropertyExprs = rawPropertyExprs.map((expr) =>
      getExpr(defaultTemplate, {
        NAUTILUS_FX_NAME: effectName,
        PROPERTY_EXPRESSION: expr,
      }),
    );
    const finalSelectorExprs = rawSelectorExprs.map((expr) =>
      getExpr(nautilus.expression.text.template, {
        VERSION: nautilus.version,
        NAUTILUS_FX_NAME: effectName,
        PROPERTY_EXPRESSION: expr,
      }),
